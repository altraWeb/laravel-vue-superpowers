...
def test_get_returns_default_value(cli):
    ...
    result = cli("get", "visual_companion_default")
...
def test_user_overlay_overrides_default(cli, user_config_dir):
    (user_config_dir / "config.yaml").write_text("teamcity_always: false\n")
    result = cli("get", "teamcity_always")
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "false"


def test_project_overlay_overrides_user(cli, user_config_dir, project_cwd):
    (user_config_dir / "config.yaml").write_text('visual_companion_default: "off"\n')
    (project_cwd / ".laravel-vue-superpowers.yaml").write_text('visual_companion_default: "ask"\n')
    result = cli("get", "visual_companion_default")
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "ask"
...
def test_validate_invalid_enum_value_fails(cli, user_config_dir):
    (user_config_dir / "config.yaml").write_text("visual_companion_default: nope\n")
    result = cli("validate")
    assert result.returncode == 3
...
def test_show_outputs_yaml_with_source_comments(cli, user_config_dir):
    (user_config_dir / "config.yaml").write_text('visual_companion_default: "ask"\n')
    result = cli("show")
    assert result.returncode == 0, result.stderr
    # Each top-level key gets a [defaults|user|project] tag
    assert "visual_companion_default: ask  # [user]" in result.stdout
    assert "teamcity_always: true  # [defaults]" in result.stdout


def test_show_marks_project_keys(cli, user_config_dir, project_cwd):
    (project_cwd / ".laravel-vue-superpowers.yaml").write_text('visual_companion_default: "ask"\n')
    result = cli("show")
    assert "visual_companion_default: ask  # [project]" in result.stdout
...
def test_init_creates_user_config_with_commented_defaults(cli, user_config_dir):
    ...
    assert '# visual_companion_default: "on"' in content
...
def test_get_with_yaml_list_overlay_falls_back_safely(cli, user_config_dir):
    ...
    result = cli("get", "visual_companion_default")
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "on"  # falls back to defaults
...
def test_warn_appends_to_errors_log(cli, user_config_dir):
    ...
    (user_config_dir / "config.yaml").write_text("key: {broken yaml\n")
    cli("get", "visual_companion_default")  # triggers parse error + warn
